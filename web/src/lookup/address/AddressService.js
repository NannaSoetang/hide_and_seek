...
const ADDRESS_DETAIL_ENDPOINT = 'https://api.dataforsyningen.dk/adresser'
...
function toSuggestion(item) {
  if (typeof item === 'string') return { id: null, text: item, lon: null, lat: null }
...
export async function searchAddresses(query, { signal } = {}) {
  ...
  for (const endpoint of AUTOCOMPLETE_ENDPOINTS) {
    try {
      const payload = await fetchJson(buildUrl(endpoint, { q: query, fuzzy: '' }), { signal })
      receivedResponse = true
      const candidates = Array.isArray(payload) ? payload : (payload?.resultater || payload?.hits || [])
...
export async function resolveAddressToCoordinates(suggestion) {
  ...
  if (!suggestion.id) throw new Error('Could not find coordinates for the selected address.')

  const payload = await fetchJson(buildUrl(ADDRESS_DETAIL_ENDPOINT, { id: suggestion.id }))
  const item = Array.isArray(payload) ? payload[0] : payload
  const coords = item?.adgangsadresse?.adgangspunkt?.koordinater
  if (Array.isArray(coords) && coords.length >= 2) {
    return { lat: Number(coords[1]), lon: Number(coords[0]), label: suggestion.text }
  }
