...
If Metro/S-tog source extraction logic changes, also run:

```bash
uv run python scripts/build_metro_data.py
uv run python scripts/build_s_tog_data.py
```
...
4. Write deterministic GeoJSON output to both data/derived and web/public/data.

Transit layers are generated by scripts/build_metro_data.py and scripts/build_s_tog_data.py.
